...
**① 开箱即用（推荐新手）**：初始化向导第 5 步选「加载示例知识库」——一键导入 `seed/` 里的**真实示例**（从你的 Obsidian 知识库 `FDogeLover/Obsidian` 精选的 AI 科技动态：3 篇来源 + 3 个实体 + 2 个概念，含 source→entity→concept 双链），直接就能看到完整工作流与知识图谱关系网。

**② 从零开始**：向导选「从空库开始」——只建目录结构与模板，之后自行在「采集」页收录内容。

> 示例内容由 `scripts/seed_from_obsidian.py` 从 Obsidian 仓库抓取精选条目生成，随仓库分发；用户数据存于 `data/`（不入库、不随仓库）。若需把 Obsidian 里更多内容同步为示例，可调整该脚本的 `FILES` 列表后重跑。
...
|---|---|
| 总览 | 笔记/主题/标签统计 + 快速入口 |
| 采集 | 抓取链接 / 粘贴正文 / **AI 方向采集** → 自动规范入库（AI 自动提炼实体/概念双链，原文保留 raw_text） |
| 整理 | 全库/单篇去重、jieba 抽关键词、规则打标签、摘要、自动建实体/概念骨架页 |
| 报告 | 数据统计（近7日趋势/主题分布）+ 内容摘要 + 知识结构三视图，可导出 Markdown |
| 笔记库 | 检索（标题/正文/标签/关键词）+ 主题/标签过滤 + 详情 + 整理/删除 |
| 知识结构 | 由笔记库双链实时聚合的知识图谱（实体/概念/来源分类渲染、聚焦高亮、详情面板） |
| 任务中心 | 长任务进度轮询 |
| 设置 | 数据目录 / 主题 / 标签 / **LLM API（三协议）/** **定时调度（cron）** |
| 初始化向导 | 建目录结构与模板（示例知识库 / 空库两种），幂等 |

**定时调度（二期）**：在「设置 → 定时调度」配置 cron 表达式（`分 时 日 月 周`），后台调度器随服务启动，按计划自动执行 AI 采集 / 全库整理 / 日报，可启停、删改，无需改动业务模块。
